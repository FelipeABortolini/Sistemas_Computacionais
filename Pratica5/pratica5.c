	...
	while(1){
		switch(state){
			...
			case HUNGRY:
                ...
                int fork_left_getted = pthread_mutex_trylock(dt->fork_left);

                if(fork_left_getted == 0){
                    int fork_right_getted = pthread_mutex_trylock(dt->fork_rigth);

                    if(fork_right_getted == 0){
                        state = EATING;
                    } else {
			...
			case EATING:
				...
				sleep(rand()%10);
                pthread_mutex_unlock(dt->fork_left);
                pthread_mutex_unlock(dt->fork_rigth);
				state = THINKING;
				break;
		}
	}
