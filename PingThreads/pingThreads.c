...
void getFromBuf(char *str){
    sem_wait(&scons); // Semáforo fechado ou aberto para executar. Se cons for "0", semáforo está fechado.
    pthread_mutex_lock(&m);
    strncpy(str, buffer[out], STRLEN);
    out = (out + 1) % BUFFERSZ;
    count--;
    pthread_mutex_unlock(&m);
    sem_post(&sprod); // Avisa o produtor que tirou do buffer
}

void setToBuf(char *str){
    sem_wait(&sprod); // Semáforo fechado ou aberto para executar. Se cons for "0", semáforo está fechado.
    pthread_mutex_lock(&m);
    strncpy(buffer[in], str, STRLEN);
    in = (in + 1) % BUFFERSZ;
    count++;
    pthread_mutex_unlock(&m);
    sem_post(&scons); // Avisa o consumidor que colocou no buffer
}
